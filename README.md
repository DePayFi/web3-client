...
or if you want to make a request with unnamed params:
...
##### Solana: request account balance
...
Returns latest `slot` (`finalized` per default).

```javascript
import { request } from '@depay/web3-client'

request('solana://latestBlockNumber').then((latestBlockNumber)=>console.log(latestBlockNumber))
```

```javascript
import { request } from '@depay/web3-client'

request('solana://latestBlockNumber', params: 'processed').then((latestBlockNumber)=>console.log(latestBlockNumber))
```
...
getAmountsOut() // makes the request to RPC
...
```javascript
import { estimate } from '@depay/web3-client'
...
// BigNumber {_hex: '0x86fc', _isBigNumber: true}
```
...
In order to set RPC provider endpoints you can use `setProviderEndpoints`:
